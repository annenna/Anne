...
namespace AlbumCoverMatchGame
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        {
            foreach (var item in await parent.GetFilesAsync())// kõik mp3
            {
                if (item.FileType == ".mp3")
                    list.Add(item);
            }

            foreach (var item in await parent.GetFoldersAsync())// kõik folderid
            {
        ...
        private async void SongGridView_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            ArtistTextBlock.Text = string.Format("Peformed by: {0}", correctSong.Artist);
            ...
            if (_round >= 5)
            {
                InstructionTextBlock.Text = string.Format("Game over... You scored: {}", _totalScore);
        ...
        private async Task<ObservableCollection<StorageFile>> SetupMusicList()
        {
            ...
            await RetrieveFilesInFolders(allSongs, folder);// parent folder 
            return allSongs;
        }

        private async Task PrepareNewGame()
        {
            Songs.Clear();// et eelmise mängu laulud ei segaks

            // 1. Choose random songs from library
            var randomSongs = await PickRandomSongs(AllSongs);

            // 2. Pluck off meta data from selected songs
